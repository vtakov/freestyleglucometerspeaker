...
try:
    while True:
        ...
        color_string = 'Color: red={0} green={1} blue={2} clear={3}'.format(r, g, b, c)

        # Print out color temperature.
        if color_temp is None:
            color_string += ' Too dark to determine color temperature!'
        else:
            color_string += ' Color Temperature: {0} K'.format(color_temp)

        # Print out the lux.
        color_string += ' Luminosity: {0} lux'.format(lux)
        print(color_string)
        sleep(1)
